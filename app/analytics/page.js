...
// // 1. Internal component to handle search params safely
// function AnalyticsContent() {
//   const searchParams = useSearchParams();
//   const rawId = searchParams.get("device") || "9"; // Get device from URL
...
//   // Logic to map the ID to the correct API identifier
...
//   // Updated useEffect to trigger whenever deviceId changes
//   useEffect(() => {
...
//         if (response.ok) {
//           setRawData(result);
//           // Auto-sync: Set calendar to the date of the actual data found
...
//     };
//     fetchData();
//   }, [deviceId]); // Re-run when device changes in URL

//   const processed = useMemo(() => {
//     const allPoints = rawData.points || [];
    
//     // 1. IMPROVED FILTERING (Day, Week, Month)
...
//         startOfWeek.setDate(target.getDate() - 7);
//         return pDate >= startOfWeek && pDate <= new Date(target).setHours(23, 59, 59);
//       } else if (viewRange === "month") {
//         return pDate.getMonth() === target.getMonth() && pDate.getFullYear() === target.getFullYear();
//       }
//       return true;
//     });
...
//       ...item,
//       // If week/month view, show Date + Time, else just Time
...
//           const valKwh = Math.max(0, (endE - startE) / 1000);
          
//           sessions.push({
//             name: `Sess ${sessions.length + 1}`,
//             kwh: valKwh,
//             // Pre-calculate display values for the bars
//             energy: valKwh < 1 ? valKwh * 1000 : valKwh,
//             unit: valKwh < 1 ? "Wh" : "kWh"
//           });
...
//     // Determine Global Unit for the Session Chart (based on max value)
//     const maxSession = Math.max(...sessions.map(s => s.kwh), 0);
//     const sessionChartUnit = maxSession < 1 ? "Wh" : "kWh";

//     // Normalize all sessions to the chart's global unit
...
//     // sessions.energy is already in kWh from the previous step
//     const totalKwhValue = sessions.reduce((acc, curr) => acc + curr.energy, 0);
...
//     if (totalKwhValue < 1) {
//       // If less than 1 kWh, convert to Wh for better readability (e.g., 0.216 kWh -> 216 Wh)
//       displayValue = (totalKwhValue * 1000).toFixed(2);
//       displayUnit = "Wh";
//     } else {
//       displayValue = totalKwhValue.toFixed(3);
//       displayUnit = "kWh";
//     }
...
//       sessionUnit: sessionChartUnit, // Pass this to the ChartCard
//       hasData: true, 
//       totalKwh: displayValue,
//       unit: displayUnit
//     };
...
//           <ChartCard title="Current Output (Amperes)" data={processed.chartData} dataKey="current" color="#3B82F6" icon={<Activity size={20} />} iconBg="bg-blue-600" hasData={processed.hasData} />
//           <ChartCard title="Inlet Voltage (V)" data={processed.chartData} dataKey="voltage" color="#8B5CF6" icon={<Zap size={20} />} iconBg="bg-purple-600" hasData={processed.hasData} />
//           <ChartCard title="Battery Health (SoC %)" data={processed.chartData} dataKey="soc" color="#10B981" icon={<Battery size={20} />} iconBg="bg-emerald-600" hasData={processed.hasData} isArea />
//           <ChartCard title="Energy Consumed per Session" data={processed.sessionData} dataKey="energy" color="#6366f1" icon={<Clock size={20} />} iconBg="bg-indigo-600" hasData={processed.hasData} isBar />
//         </div>
//       </div>
//     </div>
//   );
...
// // 2. Main export wrapped in Suspense (Required for useSearchParams in Next.js)
...
//         </div>
//         {isBar && hasData && (
//           <Badge variant="outline" className="text-[10px] font-bold text-slate-400 uppercase">UNIT: {unit}</Badge>
//         )}
//       </div>

//       <div className="flex-1 relative">
//         <ResponsiveContainer width="100%" height="100%">
//           {/* Apply a bottom margin to the specific chart type so labels aren't cut off */}
//           {isArea ? (
//             <AreaChart data={data} margin={{ bottom: 60 }}>
//                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
//                <XAxis 
//                   dataKey="displayTime" 
//                   tick={{ fontSize: 10, fontWeight: 700, fill: '#64748b', angle: -90, textAnchor: 'end' }} 
//                   height={70}
//                   interval={viewRange === "day" ? 4 : "preserveStartEnd"}
//                   axisLine={false} 
//                   tickLine={false} 
//                />
//                <YAxis axisLine={false} tickLine={false} tick={{fontSize: 12, fill: '#64748b'}} />
//                <Tooltip />
//                <Area type="monotone" dataKey={dataKey} stroke={color} fillOpacity={0.1} strokeWidth={3} />
//             </AreaChart>
//           ) : isBar ? (
...
//               <YAxis axisLine={false} tickLine={false} tick={{fontSize: 12, fill: '#64748b'}} />
//               <Tooltip formatter={(value) => [`${value.toFixed(2)} ${unit}`, "Energy"]} />
