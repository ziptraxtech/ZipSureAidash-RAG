...
//     // Creating mock sparkline data from the filtered set
//     const sparkline = filtered.slice(-10).map((p, i) => ({ value: p.current || 0 }));
//     const isOnline = (new Date() - new Date(last.datetime)) / 1000 < 600;
//     return {
...
//         // Station must be Online AND drawing > 2.0A to be counted as 'Charging'
//         charging: (isOnline && (last.current || 0) > 2.0) ? 1 : 0,
//         maintenance: 0
//       }
//     };
...
//   { title: 'Energy (End)', value: metrics ? `${metrics.endEnergy} kWh` : '--', icon: <Gauge size={20} />, trend: 'Total', color: 'cyan', lineColor: '#06b6d4', data: [{v:0.1},{v:0.2},{v:0.3},{v:0.38}] },
...
//               <p className="text-slate-500 font-medium">Monitor and manage your EV charging stations in real-time</p>
//             </div>
...
//           {stats.map((stat, i) => (
//             <Card key={i} className="p-6 rounded-[2rem] border-none shadow-xl shadow-slate-200/50 bg-white flex flex-col justify-between hover:shadow-2xl hover:scale-[1.02] transition-all duration-300">
//               <div className="flex justify-between items-start mb-4">
//                 {/* Dynamic Icon Background based on color prop */}
...
//                       <Line 
//                         type="monotone" 
//                         dataKey={stat.data[0]?.v !== undefined ? "v" : "value"} 
//                         stroke={stat.lineColor} // This adds the color to the lines
//                         strokeWidth={3} 
//                         dot={false}
...
//   <Card className="lg:col-span-1 rounded-[2.5rem] border-none shadow-2xl shadow-slate-200/50 overflow-hidden bg-white flex flex-col">
...
//     {/* Content - Increased padding for a premium feel */}
...
//   <Card className="lg:col-span-2 rounded-[2.5rem] border-none shadow-2xl shadow-slate-200/50 overflow-hidden bg-white">
...
//         </span>
//       </div>
//       <Badge variant="secondary" className="bg-slate-200 text-slate-700 font-bold px-4 py-1.5 rounded-lg border-none">
//         New Delhi, India
//       </Badge>
//     </div>
